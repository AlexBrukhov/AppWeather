...
import alexbrukhov.appweather.BuildConfig
import alexbrukhov.appweather.DialogManager
import alexbrukhov.appweather.MainViewModel
import alexbrukhov.appweather.adapters.VpAdapter
import alexbrukhov.appweather.adapters.WeatherHourModel
import alexbrukhov.appweather.adapters.WeatherDayModel
import alexbrukhov.appweather.data.WeatherApi
import alexbrukhov.appweather.databinding.FragmentMainBinding
...
class MainFragment : Fragment() {
    ...
    private fun updateCurrentCard() = with(binding) {
        model.liveDataCurrent.observe(viewLifecycleOwner) {
            ...
            Picasso.get().load("https:" + it.condition.icon).into(imWeather)
        }
    }
    ...
    private fun requestWeatherData(city: String) {
        ...
        val moshi = Moshi.Builder()
            ...
            .build()
        val moshiConverter = MoshiConverterFactory
            .create(moshi)
        ...
        model.viewModelScope.launch {
            val res = service.getForecast(
                q = city,
                days = 8,
                aqi = "no",
                alerts = "no"
            )
            val weathers = arrayListOf<WeatherDayModel>()
            val city = res.location.name

            res.forecast.forecastday.forEach {
                weathers.add(
                    WeatherDayModel(
                        city = city,
                        time = it.date,
                        condition = it.day.condition,
                        currentTemp = it.day.avgtemp_c.toString(),
            ...
            model.liveDataList.value = weathers
        }
